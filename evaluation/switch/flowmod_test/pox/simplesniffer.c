...
//      printf("Already recorded DIP \n");
   ...
   temp = ntohl(dip) >> 24;
   //printf ("temp %d\n",temp%256);
   if (temp%256 != 192) return;
   hash[ntohl(dip)%8000] = 1;
   //printf(": [%s -> %s] \n", inet_ntoa(srcip),inet_ntoa(dstip));
   globaldst[ntohl(dip)%8000] = dip;
   sec[ntohl(dip)%8000] = pkthdr->ts.tv_sec; //cur_time.tv_sec;//s;
   msec[ntohl(dip)%8000] = pkthdr->ts.tv_usec;//cur_time.tv_usec; //usec;
   c++;
   struct in_addr dst = {0};
   dst.s_addr = dip;

   printf("sec: %ld, msec: %ld \n", pkthdr->ts.tv_sec, pkthdr->ts.tv_usec);
   //printf("dst: %s sec %ld usec %ld\n", inet_ntoa(dst), pkthdr->ts.tv_sec, pkthdr->ts.tv_usec);
//   if (c % 50 == 0) {
