...
class LearningSwitch (object):
  ...
  def __init__ (self, connection, transparent):
    ...
    print 'INFO: add a default rule... I am slice 2(BCM only)'


    
    #-------------------------
    # (note that flow_mods match all flows by default)
    os.system('sudo ip netns exec receiver ./simplesniffer em4 170&')
    os.system('sudo ip netns exec sender bash ../pktgen/jpktgen.conf.1-1-flow-dist.sh &')
    time.sleep(5)
    y = 0
    print 'INFO: starting sending flow mod...'
    for k in xrange(1,171):#the number of rules to install
       #insert first
       msg = of.ofp_flow_mod()
       ...
       msg.idle_timeout = 0
       msg.hard_timeout = 0
       msg.actions.append(of.ofp_action_output(port = OUT_PORT+1))
